...
function createHeadingElement(
  ...
  try {
    if (nexthead) {
      ...
      toc = toc.appendChild(doc.createElement('ul'))
      if (headingLevel > 1) {
    ...
    if (element) {
      const child = toc.appendChild(doc.createElement('div'))
      child.classList.add(`toc-title-${headingLevel}`, `toc-title`)
      const icon = child.appendChild(doc.createElement('i'))
      ...
      const link = child.appendChild(doc.createElement('a'))
      if (element.textContent) {
        link.appendChild(doc.createTextNode(element.textContent))
      }
      link.href = '#' + element.id
    }
  } catch {
    console.warn('append toc child error')
  }
  return toc
}
...
function appendTocElement(
  doc: Document,
  element: Element,
  currTocNode: TocNode | null,
  headingLevel: number
): TocNode | null {
  if (!currTocNode) return null
  ...
  if (headingLevel <= currTocNode.headingLevel) {
    ...
    while (currTocNode) {
      ...
      tempTocNode.headingLevel = 0
      tempTocNode.preLevel = 0
    }
    if (currTocNode) appendTocElement(doc, element, currTocNode, headingLevel)
    return currTocNode
  }
  ...
  if (headingLevel > currTocNode.preLevel) {
    ...
    while (headingLevel < currTocNode.preLevel) {
      if (currTocNode.toc) currTocNode.toc = currTocNode.toc.parentElement
      if (currTocNode.toc) currTocNode.toc = currTocNode.toc.parentElement
      currTocNode.preLevel--
    }
    if (currTocNode.toc) currTocNode.toc = currTocNode.toc.parentElement
    currTocNode.toc = createHeadingElement(doc, element, currTocNode.toc, headingLevel, false)
  }

  if (currTocNode.parentNode) appendTocElement(doc, element, currTocNode.parentNode, headingLevel)
  return currTocNode
}

function renderTocElements(parent: HTMLElement, settings: TocOptions): HTMLElement[] {
  const test = settings.compatible ? toExpression(settings.compatible) : undefined

  let result: HTMLElement[] = []
  let headingLevel = 0
  let currTocNode: TocNode | null = null
  let firstHeadOne = false
  let totalToc: HTMLElement | null = null
  ...
  result.push(svgs)
  const elements = parent.querySelectorAll(headings.join(',') + (test ? ',p' : ''))
  elements.forEach((element) => {
    if (
      element.textContent &&
      ((element.tagName === 'P' && test && test.test(element.textContent)) ||
        (element.id && element.id === 'imark-toc-head'))
    ) {
      ...
      if (!firstHeadOne) {
        ...
        imarkToc = document.createElement('div')
        imarkToc.classList.add('imark-toc')
        totalToc = imarkToc
        ...
        let locale = document.body.parentElement?.lang
        if (locale) {
        ...
        if (settings.title?.length) {
          tocTitle = settings.title
        }

        const swtichToc = document.createElement('div')
        ...
        imarkToc.id = 'total-imark-toc'
        imarkToc.classList.add('total-toc')
        imarkToc.innerHTML = `<div class="toc-pin"><span class="toc-pin-text">${tocTitle}</span><div class="toc-fold-wrap"><div id="toc-fold-icon" class="toc-fold toc-icon" data-name="toc-expand" title="${tocToggle + tocTitle}"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-toc"></use></svg></div><div id="toc-close-icon" class="toc-close toc-icon" title="${hideTitle + tocTitle}"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-chapter-s"></use></svg></div></div></div>`
      } else {
        imarkToc = document.createElement('div')
        imarkToc.classList.add('imark-toc')
      }
      let tocView = document.createElement('div')
      tocView.className = 'toc-view'
      imarkToc.appendChild(tocView)
      let tocNode: TocNode = {
        parentNode: null,
        toc: tocView,
      ...
      if (currTocNode) {
        ...
        if (headingLevel > currTocNode.headingLevel) {
          ...
          let prev: TocNode | null = currTocNode
          while (prev && headingLevel < prev.headingLevel) {
      ...
      if (!firstHeadOne) {
        ...
        element.parentNode?.replaceChild(imarkToc, element)
      }
      return
    }
    ...
    if (element['tagName']) {
      headingLevel = headings.indexOf(element['tagName'].toLowerCase()) + 1
      if (headingLevel > 0 && currTocNode) {
        currTocNode = appendTocElement(document, element, currTocNode, headingLevel)
      }
    }
  })

  if (totalToc) {
    result.push(totalToc)
  }

  // Final, clear all toc node in node list.
  while (currTocNode) {
    let tempTocNode: TocNode = currTocNode
    currTocNode = tempTocNode.parentNode
    // Clean toc node.
    tempTocNode.parentNode = null
    tempTocNode.toc = null
...
function loadTocEvent() {
  const totalToc: HTMLElement | null = document.querySelector('#total-imark-toc')
  if (!totalToc) return
  ...
  if (divider) {
    ...
    function onDrag(e: MouseEvent) {
      if (startX > 0) {
        ...
        if (moveX != 0) {
          ...
          // 确保分割条宽度在合理范围内
          if (newWidth >= 5 && newWidth <= document.documentElement.clientWidth - 5 && totalToc) {
            // 调整右侧分割栏的flex值以保持比例
    ...
    function startDragging(e: MouseEvent) {
      ...
      startX = e.clientX
      // 分割条的初始宽度
      startWidth = totalToc ? totalToc.clientWidth : 0
      // 防止文本选中
  ...
  const tocFoldIcon: HTMLElement | null = document.querySelector('#toc-fold-icon')
  if (tocFoldIcon) {
    tocFoldIcon.parentElement
    tocFoldIcon.onclick = (e: MouseEvent) => {
  ...
  const tocCloseIcon: HTMLElement | null = document.querySelector('#toc-close-icon')
  if (tocCloseIcon) {
    tocCloseIcon.onclick = (e: MouseEvent) => {
      const totalToc = document.querySelector('#total-imark-toc')
  ...
  const tocSwitch: HTMLElement | null = document.querySelector('#toc-switch-button')
  if (tocSwitch) {
    tocSwitch.onclick = (e: MouseEvent) => {
  ...
  chapterIcons.forEach((e: Element) => {
    const icon = e as HTMLElement
    icon.onclick = (e: MouseEvent) => {
      ...
      const iconUse = target.querySelector('use')
      if (iconUse) {
 ...
 *   Transform.
 */
export function renderToc(): Render {
  /**
   * Transform.
   *
