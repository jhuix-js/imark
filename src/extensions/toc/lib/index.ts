...
function createHeadingElement(
  ...
  try {
    ...
    if (element) {
      ...
      icon.innerHTML =
        '<svg class="icon" aria-hidden="true"><use xlink:href="#icon-chapter-l"></use></svg>'
    ...
    console.warn('append child error:', wrapper)
  }
  return toc
}
...
function renderTocElements(wrapper: HTMLElement, test: RegExp) {
  ...
  const svgs = wrapper.ownerDocument.createElement('div')
  ...
  wrapper.parentElement?.appendChild(svgs)
  const elements = wrapper.querySelectorAll(headings.join(','))
  elements.forEach((element) => {
    if (element.textContent && test.test(element.textContent)) {
      ...
      if (!firstHeadOne) {
        ...
        imarkToc = wrapper.ownerDocument.createElement('div')
        imarkToc.classList.add('imark-toc')
        totalToc = imarkToc

        const divider = wrapper.ownerDocument.createElement('div')
        divider.id = 'divider'
        ...
        let locale = wrapper.ownerDocument.body.parentElement?.lang
        if (locale) {
        ...
        imarkToc.innerHTML = `<div class="toc-pin"><span class="toc-pin-text">${tocTitle}</span><div class="toc-fold-wrap"><div id="toc-fold-icon" class="toc-fold toc-icon" data-name="toc-expand"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-toc"></use></svg></div><div id="toc-close-icon" class="toc-close toc-icon"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-chapter-s"></use></svg></div></div></div>`
      } else {
        imarkToc = wrapper.ownerDocument.createElement('div')
        imarkToc.classList.add('imark-toc')
      }
  ...
  if (totalToc) {
    wrapper.parentElement?.appendChild(totalToc)
  }
...
function loadTocEvent() {
  const totalToc: HTMLElement | null =
    document.querySelector('#total-imark-toc')
  if (!totalToc) return
    ...
    function onDrag(e: MouseEvent) {
      if (startX > 0) {
        ...
        if (moveX != 0) {
          ...
          if (
            newWidth >= 5 &&
            newWidth <= document.documentElement.clientWidth - 5 &&
            totalToc
          ) {
            // 调整右侧分割栏的flex值以保持比例
  ...
  const tocFoldIcon: HTMLElement | null =
    document.querySelector('#toc-fold-icon')
  if (tocFoldIcon) {
    tocFoldIcon.onclick = (e: MouseEvent) => {
      const chapters = document.querySelectorAll('.total-toc ul')
      const icons = document.querySelectorAll(
        '.total-toc ul:not(.toc-main) .toc-multi-chapter use'
      )
      if (tocFoldIcon.dataset.name === 'toc-expand') {
        chapters.forEach((element) => {
          if (
            !element.classList.contains('toc-main') &&
            !element.classList.contains('toc-level-2')
          ) {
  ...
  const tocCloseIcon: HTMLElement | null =
    document.querySelector('#toc-close-icon')
  if (tocCloseIcon) {
  ...
  const tocSwitch: HTMLElement | null =
    document.querySelector('#toc-switch-button')
  if (tocSwitch) {
  ...
  const chapterIcons = document.querySelectorAll(
    '.total-toc .toc-multi-chapter'
  )
  chapterIcons.forEach((e: Element) => {
    ...
    icon.onclick = (e: MouseEvent) => {
      ...
      if (iconUse) {
        const href = iconUse.getAttributeNS(
          'http://www.w3.org/1999/xlink',
          'href'
        )
        if (href === '#icon-expand') {
...
export function renderToc(): Render {
  ...
  return function (tree: HTMLElement, option?: TocOptions) {
    const settings = {
      heading: '\\[Table[ -]Of[ -]Contents\\]|\\[目录\\]|\\[TOC\\]',
      ...option
    }
    const headTest = settings.heading
      ? toExpression(settings.heading)
      : undefined
    if (headTest) {
      renderTocElements(tree, headTest)
      loadTocEvent()
    }
  } as Render
}
